...
public class SwerveDriveSubsystem extends SubsystemBase {
   ...
   *                    {@link com.pathplanner.lib.PathPlanner#loadPathGroup}
   *                    function
   * @return Returns a sendable command to follow the given path. Will return null
   *         if auto builder has not been initialized or if path is not a file
   */
  public CommandBase loadPathPlannerCommand(String path, PathConstraints constraints) {
    ...
    if (autoBuilder == null || !new File(Filesystem.getDeployDirectory(), "pathplanner/" + path + ".path").isFile()) {
      ...
      // Creates the trajectories in the path and returns them as an autonomous
      // command group
      List<PathPlannerTrajectory> pathGroup = PathPlanner.loadPathGroup(path, constraints);
   ...
   *                         color.
   */
  public void initAutoBuilder(Map<String, Command> eventMap, PIDConstants translation, PIDConstants rotation,
